...
MAXIMUM_DEPOSIT=10000000000000000000
DEADLINE_UNIX_TIMESTAMP=$(date -d "2022-04-19 20:35:00" +%s)
...
deploy() {
    erdpy --verbose contract deploy --project=${PROJECT} --recall-nonce \
        ...
        --outfile="deploy-devnet.interaction.json" --send --proxy=${PROXY} --chain=${CHAIN} || return
...
deploySimulate() {
    ...
    --pem=${ALICE} \
    --arguments ${TARGET} ${MINIMUM_DEPOSIT} ${MAXIMUM_DEPOSIT} ${DEADLINE_UNIX_TIMESTAMP} \
    --outfile="simulate-devnet.interaction.json" --simulate --proxy=${PROXY} --chain=${CHAIN} || return

    TRANSACTION=$(erdpy data parse --file="simulate-devnet.interaction.json" --expression="data['simulation']['execution']['result']['hash']")
    ADDRESS=$(erdpy data parse --file="simulate-devnet.interaction.json" --expression="data['contractAddress']")
    RETCODE=$(erdpy data parse --file="simulate-devnet.interaction.json" --expression="data['simulation']['execution']['result']['status']")
    RETMSG=$(erdpy data parse --file="simulate-devnet.interaction.json" --expression="data['simulation']['execution']['result']['returnMessage']")
...
claimFunds() {
    erdpy --verbose contract call ${ADDRESS} --recall-nonce --keyfile=${KEYSTORE_FILE} --passfile=${PASS_FILE} --gas-limit=10000000 \
        --function="claim" \
...
getmaximumDeposit() {
    erdpy --verbose contract query ${ADDRESS} --function="maximumDeposit" --proxy=${PROXY}
}

getminimumDeposit() {
    erdpy --verbose contract query ${ADDRESS} --function="minimumDeposit" --proxy=${PROXY}
}
